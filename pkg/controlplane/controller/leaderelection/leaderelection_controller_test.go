...
func TestReconcileElectionStep(t *testing.T) {
	...
	tests := []struct {
		...
		// },
		{
			name:    "candidates exist, lease exists, candidate with pickme strategy should set preferredHolder regardless of the enumlated version",
			leaseNN: types.NamespacedName{Namespace: "default", Name: "component-A"},
			candidates: []*v1alpha1.LeaseCandidate{
				{
					ObjectMeta: metav1.ObjectMeta{
					...
					Spec: v1alpha1.LeaseCandidateSpec{
						...
						BinaryVersion:       "1.19.0",
						RenewTime:           ptr.To(metav1.NewMicroTime(fakeClock.Now())),
						PreferredStrategies: []v1.CoordinatedLeaseStrategy{v1.PickMe, v1.OldestEmulationVersion},
					},
				},
				{
					ObjectMeta: metav1.ObjectMeta{
			...
			expectedStrategy:        ptr.To[v1.CoordinatedLeaseStrategy]("PickMe"),
			expectedRequeue:         true,
			expectedError:           false,
		},
		// {
